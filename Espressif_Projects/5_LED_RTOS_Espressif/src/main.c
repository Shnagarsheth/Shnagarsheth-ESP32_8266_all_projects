 ...
 * Important Notes: 1. 25 priority levels can be set in FreeRTOS of ESP32. 
 *                  2. vTaskDelay is a non-blocking mode of delay in which it allows other tasks to run.
 *                  3. while(1) cannot be empty otherwise "Wathcdog" will reset ESP32. 
 *                  4. void app main() is by default considered to be a task with priority level 0. 
 **/
