...
void ADE7758::Init(void) {
  ...
  digitalWrite((AFECS), LOW); //disabled by default
	// SPI Init
...
void ADE7758::enableChip(void){
	...
	GPIO.out_w1ts = ((uint32_t)1 << AFECS);
}
...
void ADE7758::disableChip(void){
	...
	GPIO.out_w1tc = ((uint32_t)1 << AFECS);
}
...
//Get waveform sapling
long ADE7758::getWFORM(void){

	long lastupdate = 0;
	char t_of = 0;
	// resetStatus(); // Clear all interrupts
	lastupdate = millis();
	while(!(getStatus() & WFSM))   // wait IRQ Detection
	...
	return read24W(WFORM);
	}
}

//-------Set Peak Voltage Value for Detection-----// 
/**
 * @brief This function will set the peak voltage value in VPINTLVL 
 * and PFIRQSEL in MMODE to enable Peak Interrupt Detection. 
 * Set PKI and PKV in MASK REGISTER (0x18) to generate an Interrupt 
 * when the absolute values of the voltage waveform samples in the selected phase exceeds the 
 * value in VPINTLVL.
 * @param char containing Peak Voltage
 * @author Shaival Nagarsheth
 */

void ADE7758::setPVoltageDetect(char pvalue){
	float PV_CAL=0.314;
	char peakV;
	peakV = (char)((PV_CAL)*((((float)(pvalue))*1.414)+5));
	write8(VPINTLVL, peakV);//Command 1 - write8 --> to PVINTLVL (Desired Peak Voltage Value)
	setMMode(PKIRQSEL2);	//Command 2 - MMODE register 
	setMaskInterrupts(PKV);	// Command 3 - Set--- (0x18)
}

void ADE7758::setPCurrentDetect(float pvalue){
	float PI_CAL=200;
	char peakI;
	peakI = (char)((PI_CAL)*(((pvalue*0.030)*1.414)*2));
	write8(IPINTLVL, peakI);//Command 1 - write8 --> to PVINTLVL (Desired Peak Voltage Value)
	setMMode(PKIRQSEL2);	//Command 2 - MMODE register 
	setMaskInterrupts(PKI);	// Command 3 - Set--- (0x18)
}

//-------Set Low Voltage Value for Detection-----// 
/**
 * @brief This function will set the Low voltage value in SAGLVL 
 * Set SAGA, SAGB, SAGC in MASK REGISTER (0x18) to generate an Interrupt 
 * when the absolute values of the voltage waveform samples in the selected phase goes below the 
 * value in SAGLVL.
 * @param char containing Low Voltage
 * @author Shaival Nagarsheth
 */

void ADE7758::setSVoltageDetect(char lvalue){
	float LV_CAL=0.314;
	char lowV;
	lowV = (char)((LV_CAL)*((((float)(lvalue))*1.414)+5));
	write8(SAGLVL, lowV);//Command 1 - write8 --> to SAGLVL (Desired Low Voltage Value for three phases (it's Common))
	setMaskInterrupts(SAGA || SAGB || SAGC);	// Command 3 - Activate IRQ for all three phases for SAG detection 
}

/** === getAIRMS ===
* Phase A Current RMS Value 
* To minimize 
noise, synchronize the reading of the rms register with the zero crossing 
...
*/
//int ADE7758::getFreq(char phase){
int ADE7758::getFreq(void){
//	uint8_t mmode;
//  mmode = read8(MMODE);
//  write8(MMODE,(mmode&0x11111100 )| phase);
//  delay(10);
	return read16(FREQ);
}
